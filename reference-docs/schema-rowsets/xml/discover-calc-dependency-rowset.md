...
|**QUERY**|**DBTYPE_WSTR**|Yes|For tabular models created in Integration Services, you can include a DAX query or expression to show the dependency graph for that query or expression. The QUERY restriction provides client applications with a way to determine which objects are used by a DAX query.<br /><br /> The **QUERY** restriction can be specified in XMLA or in the WHERE clause of a DMV query. See the examples section for more information.|  
