...
# DMSCHEMA_MINING_STRUCTURE_COLUMNS Rowset
  ...
  Describes the individual columns of all mining structures deployed on a server that is running Analysis Services.  
...
|**STRUCTURE_SCHEMA**|**DBTYPE_WSTR**||The unqualified schema name. Analysis Services does not support schemas, so this column is always **NULL**.|  
...
|**COLUMN_PROPID**|**DBTYPE_UI4**||The column property ID. Providers that do not associate property IDs with columns should return **NULL** in this column. SQL Server Analysis Services returns **NULL** for this column.|  
