...
description: Learn how to use the Process command to process an Analysis Services object. 
ms.date: 05/02/2018
...
## Specifying Out-of-Line Bindings  
 If the **Process** command is not contained by a **Batch** command, you can optionally specify out-of-line bindings in the [Bindings](../xmla/xml-elements-properties/bindings-element-xmla.md), [DataSource](../xmla/xml-elements-properties/datasource-element-xmla.md), and [DataSourceView](../xmla/xml-elements-properties/datasourceview-element-xmla.md) properties of the **Process** command for the objects to be processed. Out-of-line bindings are references to data sources, data source views, and other objects in which the binding exists only during the execution of the **Process** command, and which override any existing bindings associated with the objects being processed. If out-of-line bindings are not specified, the bindings currently associated with the objects to be processed are used.  
