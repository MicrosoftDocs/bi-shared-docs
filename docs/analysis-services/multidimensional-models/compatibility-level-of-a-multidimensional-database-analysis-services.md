...
|Setting|Description|  
|-------------|-----------------|  
|**1050**|This value isn't visible in script or tools, but it corresponds to databases created in [!INCLUDE[ssVersion2005](../includes/ssversion2005-md.md)], [!INCLUDE[ssKatmai](../includes/sskatmai-md.md)], or [!INCLUDE[ssKilimanjaro](../includes/sskilimanjaro-md.md)]. Any database that doesn't have **CompatibilityLevel** explicitly set implicitly runs at the **1050** level.|  
...
1.  Using SQL Server Management Studio, connect to the [!INCLUDE[ssCurrent](../includes/sscurrent-md.md)][!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] server that hosts the database.  
  
1.  Right-click the database name, point to **Script Database as**, point to **ALTER to**, and then select **New Query Editor Window**. An XMLA representation of the database opens in a new window.  
  
1.  Copy the following XML element:  
  
    ```  
    <ddl200:CompatibilityLevel>1100</ddl200:CompatibilityLevel>  
    ```  
  
1.  Paste the element after the `</Annotations>` closing element and before the `<Language>` element. The XML should look similar to the following example:  
...
1.  To run the script, select **Execute** on the Query menu or press F5.
