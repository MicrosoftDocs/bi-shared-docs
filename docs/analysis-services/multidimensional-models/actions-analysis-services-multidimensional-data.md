...
description: Learn about actions in SQL Server Analysis Services, including drillthrough actions, reporting actions, and standard actions.
ms.date: 05/02/2018
ms.prod: sql
...
-   Standard actions, which return the action element (URL, HTML, DataSet, RowSet, and other elements) that is associated with the selected section of the cube where the action occurs.  
 ...
 A query interface, such as ADOMD.NET, is used by the client application to retrieve and expose the actions to the end user. For more information see [Developing with ADOMD.NET](https://docs.microsoft.com/analysis-services/adomd/developing-with-adomd-net).  
