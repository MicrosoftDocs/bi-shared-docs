...
3.  Configure the following property settings:  
    ...
    |**Default workspace server**|**localhost**| All available instances of Analysis Services running on the local computer are included in the listbox.<br /><br />For workspace databases on a remote server, data cannot be backed up locally and the user interface may experience latency during queries.|  
